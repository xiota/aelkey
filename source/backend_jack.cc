...
bool BackendJack::ensure_client() {
  if (client_) {
    return true;
  }
  ...
  if (jack_set_process_callback(client_, &BackendJack::process_cb, this) != 0) {
  ...
  jack_set_port_registration_callback(client_, &BackendJack::port_reg_cb, this);
...
bool BackendJack::set_client_name(const std::string &name) {
...
jack_port_t *
BackendJack::create_port(const std::string &name, const char *type, unsigned long flags) {
  if (!ensure_client()) {
    return nullptr;
  }
...
void BackendJack::destroy_port(jack_port_t *port) {
  if (client_ && port) {
    jack_port_unregister(client_, port);
  }
}

std::vector<std::string> BackendJack::list_ports(const char *type, unsigned long flags) {
...
jack_port_t *BackendJack::find_port(const std::string &full_name) {
  if (!ensure_client()) {
    return nullptr;
  }
...
std::string BackendJack::port_name(jack_port_t *port) {
  if (!port) {
    return {};
  }
...
void *BackendJack::port_buffer(jack_port_t *port, jack_nframes_t nframes) {
  if (!port) {
    return nullptr;
  }
...
bool BackendJack::connect(const std::string &src, const std::string &dst) {
  if (!ensure_client()) {
    return false;
  }
...
bool BackendJack::disconnect(const std::string &src, const std::string &dst) {
  if (!client_) {
    return false;
  }
...
std::vector<std::string> BackendJack::port_connections(jack_port_t *port) {
  std::vector<std::string> result;
...
uint32_t BackendJack::midi_event_count(void *buf) {
  if (!buf) {
    return 0;
  }
...
bool BackendJack::midi_event_get(jack_midi_event_t &out, void *buf, uint32_t index) {
  if (!buf) {
    return false;
  }
...
void BackendJack::midi_clear_buffer(void *buf) {
  if (!buf) {
    return;
  }
...
jack_midi_data_t *BackendJack::midi_event_reserve(void *buf, jack_nframes_t time, size_t size) {
  if (!buf) {
    return nullptr;
  }
...
void BackendJack::port_reg_cb(jack_port_id_t port_id, int registered, void *arg) {
  auto *self = static_cast<BackendJack *>(arg);
  if (!self) {
    return;
  }
...
void BackendJack::handle_port_registration(jack_port_id_t port_id, int registered) {
  if (!client_) {
    return;
  }
