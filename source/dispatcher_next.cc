...
DispatcherNextBase::~DispatcherNextBase() {
  cleanup_fds();
}

EpollPayload *DispatcherNextBase::get_payload(int fd) const {
  auto it = pollfds_.find(fd);
  return (it != pollfds_.end()) ? const_cast<EpollPayload *>(&it->second) : nullptr;
}

void DispatcherNextBase::register_fd(int fd, uint32_t events) {
...
void DispatcherNextBase::unregister_fd(int fd) {
  auto &state = AelkeyState::instance();
...
void DispatcherNextBase::cleanup_fds() {
  auto &state = AelkeyState::instance();
...
void DispatcherNextBase::flush_deferred() {
...
void DispatcherNextBase::handle_event(EpollPayload *payload, uint32_t /*events*/) {
  if (!payload) {
    return;
  }
